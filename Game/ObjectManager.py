...
class ObjectManager:
	...
	def update(self,controlsPressed, controlsHold, mousePressed, mousePos, codingBar, viewX=0, viewY=0):
		for i in range(0,len(self.instance)):
			...
			if (mousePressed and mousePos[0] >= self.instance[i].x-viewX and mousePos[0] < self.instance[i].x+self.instance[i].xSpace*32-viewX and mousePos[1] >= self.instance[i].y-viewY and mousePos[1] < self.instance[i].y+self.instance[i].ySpace*32-viewY):
